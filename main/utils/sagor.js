...
let _langRaw;
try {
    _langRaw = readFileSync(join(global.client.mainPath, `main/languages/${global.config.language || "en"}.lang`), { encoding: 'utf-8' });
} catch (_) {
    _langRaw = readFileSync(join(global.client.mainPath, `main/languages/en.lang`), { encoding: 'utf-8' });
}
const langFile = _langRaw.split(/\r?\n|\r/);
...
for (const item of langData) {
    ...
    const head  = dotIdx !== -1 ? itemKey.slice(0, dotIdx) : itemKey;
    const key   = dotIdx !== -1 ? itemKey.slice(dotIdx + 1) : itemKey;
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async (loginError, loginApiData) => {
        ...
        try {
            ...
            global.data.botID = _uid; // Fix: store bot's own ID so modules can reference it
       ...
       if (global.config.autoRestartWhenListenMqttError === true) {
        ...
        // HOT-RELOAD: নতুন cmd/event file add হলে auto load হবে
        const _fs2 = require('fs');
        const _cmdDir = join(global.client.mainPath, 'src/cmds');
        const _evtDir = join(global.client.mainPath, 'src/events');

        function hotLoadCmd(filename) {
            if (!filename || !filename.endsWith('.js') || filename.includes('example')) return;
            const fullPath = join(_cmdDir, filename);
            if (!_fs2.existsSync(fullPath)) return;
            try {
                delete require.cache[require.resolve(fullPath)];
                const mod = require(fullPath);
                if (!mod || !mod.config || (!mod.run && !mod.handleEvent) || !mod.config.commandCategory) {
                    logger(`[ HOT-RELOAD ] ${filename} invalid format (missing config/run/commandCategory)`, '[ HOT-RELOAD ]');
                    return;
                }
                const isUpdate = global.client.commands.has(mod.config.name);
                if (!isUpdate && mod.handleEvent && !global.client.eventRegistered.includes(mod.config.name))
                    global.client.eventRegistered.push(mod.config.name);
                global.client.commands.set(mod.config.name, mod);
                logger(`[ HOT-RELOAD ] ${isUpdate ? 'Updated' : 'New cmd loaded'}: ${mod.config.name}`, '[ HOT-RELOAD ]');
            } catch (e) {
                logger(`[ HOT-RELOAD ] Failed to load ${filename}: ${e.message}`, '[ HOT-RELOAD ]');
                console.error(e.stack);
            }
        }

        function hotLoadEvt(filename) {
            if (!filename || !filename.endsWith('.js') || filename.includes('example')) return;
            const fullPath = join(_evtDir, filename);
            if (!_fs2.existsSync(fullPath)) return;
            try {
                delete require.cache[require.resolve(fullPath)];
                const ev = require(fullPath);
                if (!ev || !ev.config || !ev.config.name) {
                    logger(`[ HOT-RELOAD ] ${filename} invalid event format`, '[ HOT-RELOAD ]');
                    return;
                }
                global.client.events.set(ev.config.name, ev);
                logger(`[ HOT-RELOAD ] New event loaded: ${ev.config.name}`, '[ HOT-RELOAD ]');
            } catch (e) {
                logger(`[ HOT-RELOAD ] Failed to load event ${filename}: ${e.message}`, '[ HOT-RELOAD ]');
                console.error(e.stack);
            }
        }

        try {
            _fs2.watch(_cmdDir, { persistent: false }, (evType, filename) => {
                if (evType === 'rename' || evType === 'change') setTimeout(() => hotLoadCmd(filename), 300);
            });
            _fs2.watch(_evtDir, { persistent: false }, (evType, filename) => {
                if (evType === 'rename' || evType === 'change') setTimeout(() => hotLoadEvt(filename), 300);
            });
            logger('Hot-reload watching src/cmds/ and src/events/ for changes', '[ HOT-RELOAD ]');
        } catch (watchErr) {
            logger(`Hot-reload watch failed: ${watchErr.message}`, '[ WARN ]');
        }
    });
}
...
(async () => {
    try {
        ...
        const models = await require('../database/model')(authentication);
